...
Опциональная нативная обёртка над веб-клиентом — нужна, если хочется
**глобальные хоткеи** (мьют микро/наушников, работают даже когда окно
не в фокусе) и автостарт. Все данные (сообщения, звонки, контакты)
по-прежнему живут на сервере — десктоп ходит к нему по тем же URL'ам,
что и браузер, и не дублирует ничего локально.
...
При первом запуске десктоп спросит URL сервера. Конфиг сохраняется в
`%APPDATA%/OwnCord/owncord.config.json` (на Linux/macOS — аналогичная
папка профиля). Подробности и пер-OS сборка — в `desktop/README.md`.

**Биндинги**: Настройки → вкладка «Биндинги» (видна только в десктопе).
Сейчас доступны два действия: «Мьют микрофона» и «Глушить динамики».
Хоткей назначается записью комбинации (Ctrl/Shift/Alt + клавиша),
формат — `accelerator` Electron'а (https://electronjs.org/docs/latest/api/accelerator).
